...
class Fixed(reolinkapi.Camera, FixStreamAPIMixin):
...
async def capture_video_async(input, output, duration=10, loop=None):
    ...
    stdout = await loop.run_in_executor(ProcessPoolExecutor(), capture_video, input, output, duration)
    await asyncio.sleep(0.01)

def capture_frame(input, output):
    os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "tcp"
    os.popen(f'ffmpeg -y -i "{input}" -t 0.1 "{output}.mp4"').read()
    os.popen(f'ffmpeg -y -i "{output}.mp4" -vf "select=eq(n\,0)" -q:v 3 "{output}"').read()

async def capture_frame_async(input, output, loop=None):
    loop = loop if loop is not None else asyncio.get_event_loop()
    stdout = await loop.run_in_executor(ProcessPoolExecutor(), capture_frame, input, output)
    await asyncio.sleep(0.01)
...
async def bouncer(event):
    ...
    try:
        print(f"[FORBIDDEN] by id={user.id}; {user.first_name} {user.last_name}.")
    except:
        print(f"[FORBIDDEN] id={event.sender_id}; non-user has been forbidden.")
...
async def start_handler(event):
    ...
    print(f"[START] by id={user.id}; {user.first_name} {user.last_name}.")
    raise events.StopPropagation
...
async def toggle_handler(event):
    upload = None
    await bot(functions.messages.SetTypingRequest(peer=event.sender_id, action=types.SendMessageTypingAction()))
    user = await bot.get_entity(event.sender_id)
    ...
    await bot(functions.messages.SetTypingRequest(peer=event.sender_id, action=types.SendMessageCancelAction()))
    recipients = list(set([JASON, CHELSEA, event.sender_id]))
    #recipients = list(set([JASON, event.sender_id]))
...
async def check_handler(event):
    ...
    presets = dict(garage=PRESET_GARAGE, driveway=PRESET_DRIVEWAY, shed=PRESET_SHED, paddock=PRESET_PADDOCK)
...
async def check_handler(event):
    await bot(functions.messages.SetTypingRequest(peer=event.sender_id, action=types.SendMessageTypingAction()))
    filename = f"konami.mp4"
    try:
        camera = Fixed(CAMERA_URL, CAMERA_USER, CAMERA_PASS)
        camera.auto_movement()
        await save_video(10, filename)
        await event.reply(file=filename)
    except:
        await event.reply("error")
    await asyncio.sleep(1)
